...
// normalize SRC_PATH_GENERATED by updating \ with / , because path.join normalizes
// the path with \ on windows, but we need /, because getIcons uses / with glob.sync.

const normalizedSourcePathGenerated = SRC_PATH_GENERATED.replace(/\\/g, "/");
const iconsPromise = getIcons(normalizedSourcePathGenerated);
iconsPromise
