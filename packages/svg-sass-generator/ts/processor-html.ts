...
iconsPromise
  .then((result: string[]) => {
    const categoriesList: IconsList = createIconsObject(result);
 ...
 */
const crateHTMLIcons = (category: string, icons: icon[]): string => {
  let html = ``;
 ...
 */
function createIconsObject(iconsArray: string[]): IconsList {
...
const getViewsIds = (iconPath: string) => {
  let file;
  try {
...
export type icon = { colorScheme: string; fileName: string; states: string[] };
