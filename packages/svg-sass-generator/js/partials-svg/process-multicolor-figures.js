...
let warnings = [];
let usedColors = [];
export const processMulticolorFigures = (svgCheerio, statesJson, iconPath, LOG_PATH) => {
...
const processFigure = (index, svgFigure, statesJson) => {
...
const updateFigureFillOrStroke = (svgFigure, colorName) => {
...
const getColorName = (index, svgFigure, cssClasses, statesJson) => {
    const figureTagName = svgFigure.get(0).name; /*revisar*/
    const cssPrefix = statesJson.multicolor.cssPrefix;
    const cssClassesArray = cssClasses.split(" ");
    const colorClass = cssClassesArray.find((cssClass) => cssClass.startsWith(cssPrefix));
