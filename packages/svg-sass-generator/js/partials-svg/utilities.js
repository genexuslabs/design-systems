 ...
 */
export const getStatesObject = (STATES_PATH) => {
    try {
        // Read the file synchronously
        const statesString = readFileSync(STATES_PATH, "utf8");
        return {
            valid: true,
            info: "Ok",
...
export const saveSvgOnDisk = (svgString, iconPath, outputDirectory, colorScheme, LOG_PATH) => {
    // construct file path
    const schemeFolderName = colorScheme;
    const pathInfo = getPathInfo(iconPath);
    const fileDirectoriesPath = path.join(outputDirectory, pathInfo.categoryFolderName, schemeFolderName);
...
export const getPathInfo = (iconPath) => {
...
export const getFigureType = (svgFigure) => {
...
export const removeSpacesAndLineBreaks = (str) => {
