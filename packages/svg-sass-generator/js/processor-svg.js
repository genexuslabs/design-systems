...
import { readyToProcess } from "./partials-svg/ready-to-process.js";
...
const readyObj = readyToProcess(SRC_DIRECTORY, OUTPUT_DIRECTORY, STATES_FILENAME, LOG_PATH, numberOfArgsProvided);
if (readyObj.ready) {
    //clear
 ...
 */
function processIcons(iconsArray, statesJson) {
    let processedIconsInfo = [];
    iconsArray.forEach(iconPath => {
        ...
        if (iconType === "multicolor") {
            const svgFiguresResult = processMulticolorFigures(svgCheerio, statesJson, iconPath, LOG_PATH);
            if (svgFiguresResult.processed) {
                /* ==================
                MULTICOLOR LIGHT
                ================== */
                // Create and get svg
                const svgIconLight = createMulticolorSvg(svgFiguresResult, statesJson, "light");
                // Save svg on disk
                let lightSvgSavedOnDisk = false;
                if (svgIconLight) {
                    lightSvgSavedOnDisk = saveSvgOnDisk(svgIconLight, iconPath, OUTPUT_DIRECTORY, "light", LOG_PATH);
                }
                // Save info for the log
                saveProcessedIconInfo(processedIconsInfo, lightSvgSavedOnDisk, iconPath, "light");
                /* ==================
                MULTICOLOR DARK
                ================== */
                // Create and get svg
                const svgIconDark = createMulticolorSvg(svgFiguresResult, statesJson, "dark");
                // Save svg on disk
                let darkSvgSavedOnDisk = false;
                if (svgIconDark) {
                    darkSvgSavedOnDisk = saveSvgOnDisk(svgIconDark, iconPath, OUTPUT_DIRECTORY, "dark", LOG_PATH);
                }
                // Save info for the log
                saveProcessedIconInfo(processedIconsInfo, darkSvgSavedOnDisk, iconPath, "dark");
            }
        }
            ...
            ================== */
            // Create and get svg
            const svgIconLight = createMonochromeSvg(svgCheerio, statesJson, iconPath, "light", LOG_PATH, STATES_FILENAME);
            // Save svg on disk
            let lightSvgSavedOnDisk = false;
            if (svgIconLight.processed) {
                lightSvgSavedOnDisk = saveSvgOnDisk(svgIconLight.svgString, iconPath, OUTPUT_DIRECTORY, "light", LOG_PATH);
            }
            // Save info for the log
            saveProcessedIconInfo(processedIconsInfo, lightSvgSavedOnDisk, iconPath, "light");
            /* ==================
            ...
            ================== */
            // Create and get svg
            const svgIconDark = createMonochromeSvg(svgCheerioClone, statesJson, iconPath, "dark", LOG_PATH, STATES_FILENAME);
            // Save svg on disk
            let darkSvgSavedOnDisk = false;
            if (svgIconDark.processed) {
                darkSvgSavedOnDisk = saveSvgOnDisk(svgIconDark.svgString, iconPath, OUTPUT_DIRECTORY, "dark", LOG_PATH);
            }
            // Save info for the log
            saveProcessedIconInfo(processedIconsInfo, darkSvgSavedOnDisk, iconPath, "dark");
        }
...
const logProcessedIconsInfo = (processedIconsInfo) => {
    const processed = [];
    const notProcessed = [];
...
const saveProcessedIconInfo = (processedIconsInfo, wasSavedOnDisk, iconPath, colorScheme) => {
...
const validateIconPath = (iconPath) => {
    const segments = iconPath.split(path.sep);
    // subtract 1 to exclude the file name
    const numFolders = segments.length - 1;
    if (numFolders > 2) {
        const msg = `Please ensure that every icon.svg file is either directly inside the source folder or within a subfolder of the source folder. Files nested deeper than one level beyond the source folder will not be considered valid. ${numFolders - 1} folders found in: ${iconPath}`;
        log(msg, LOG_PATH, shouldWriteToLog);
        return false;
    }
    return true;
