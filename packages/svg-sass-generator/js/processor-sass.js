...
let monochromeCategoriesList = [];
let multicolorCategoriesList = [];
 ...
 */
export function processIconsSass(sourceDir, iconsArray) {
    const iconsCatalog = {
        monochrome: {},
        multicolor: {}
    };
...
const addIconInCatalog = (sourceDir, iconPath, iconsCatalog) => {
    let states = [];
    ...
    const iconType = svgElement.attr("data-scheme");
 ...
 */
const updateIconsCatalog = (sourceDir, iconPath, iconsCatalog, iconType, states) => {
    ...
    iconsCatalog[iconType][category][type].push({
        fileName: filename,
        states: states
    });
};
const processIconsCatalog = (iconsCatalog) => {
    // 1. process monochrome
    for (const category in iconsCatalog.monochrome) {
        processCatalogCategory("monochrome", category, iconsCatalog.monochrome[category]);
    }
    // 2. process multicolor
    for (const category in iconsCatalog.multicolor) {
        processCatalogCategory("multicolor", category, iconsCatalog.multicolor[category]);
    }
};
const processCatalogCategory = (type, categoryName, categoryIcons) => {
...
const createSassFileString = (categoryName, categoryIcons) => {
    ...
    const lightPlaceholders = createPlaceholders(categoryName, categoryIcons, "light");
    const darkPlaceholders = createPlaceholders(categoryName, categoryIcons, "dark");
...
const createPlaceholders = (categoryName, categoryIcons, scheme) => {
 ...
 */
const saveCategory = (type, categoryName) => {
    if (type === "monochrome") {
...
const saveSassOnDisk = (sassFileContent, type, categoryName) => {
